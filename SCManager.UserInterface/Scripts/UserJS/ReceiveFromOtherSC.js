    ...
    try {
        DataTables.tblInvoicesList = $('#tblInvoicesList').DataTable(
       {
           ...
           columns: [
             ...
             { "data": "FromSCName", "defaultContent": "<i>-</i>" },          
        ...
        DataTables.tblReceiveFromOtherSCDetails = $('#tblReceiveFromOtherSCDetails').DataTable(
      {
...
function GetAllOtherSCReceipt() {
    try {       
        var data = {};
        var ds = {};
        ds = GetDataFromServer("ReceiveFromOtherSC/GetAllOtherSCReceipt/", data);
...
function Edit(currentObj) {
    ...
    if ((rowData != null) && (rowData.ID != null)) {
        ...
        if (BindOtherSCReceipt(rowData.ID)) {
            ChangeButtonPatchView('ReceiveFromOtherSC', 'btnPatchReceiveFromOtherSCSettab', 'Edit');
...
function BindOtherSCReceipt(id) {
    try {
        var data = { "ID": id };
        var ds = {};
        ds = GetDataFromServer("ReceiveFromOtherSC/GetOtherSCReceiptByID/", data);
        if (ds != '') {
            ds = JSON.parse(ds);
        }
        if (ds.Result == "OK") {
...
function BindOtherSCReceiptFields(Records) {
    try {
        ...
        $("#FromSCName").val(Records.FromSCName);
...
function ResetForm() {
    var validator = $("#OtherSCReceipt").validate();
    $('#OtherSCReceipt').find('.field-validation-error span').each(function () {
...
function List() {
    try {
        ...
        ChangeButtonPatchView('ReceiveFromOtherSC', 'btnPatchReceiveFromOtherSCSettab', 'List');
        $("#HeaderID").val("");
        reset();
...
function Add() {
    ...
    ChangeButtonPatchView('ReceiveFromOtherSC', 'btnPatchReceiveFromOtherSCSettab', 'Add');
    EG_ClearTable();
    EG_AddBlankRows(5)
    $("#HeaderID").val("");
    reset();
...
function OtherSCReceiptDelete() {
    try {
        ...
        if (id != '' && id != null) {
            var data = { "ID": id };
            var ds = {};
            ds = GetDataFromServer("ReceiveFromOtherSC/DeleteOtherSCReceipt/", data);
            if (ds != '') {
                ds = JSON.parse(ds);
            }
            if (ds.Result == "OK") {
...
function OtherScReceiptDetailDelete(id, rw) {
    try {
        ...
        if (id != '' && id != null && Hid != '' && Hid != null && Hid != emptyGUID) {
            ...
            var ds = {};
            ds = GetDataFromServer("ReceiveFromOtherSC/DeleteOtherScReceiptDetail/", data);
            if (ds != '') {
                ds = JSON.parse(ds);
            }
            if (ds.Result == "OK") {
